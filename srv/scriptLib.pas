...
//  unnamedPars: integer; // this is a guessing of the number of unnamed parameters. just guessing because there's no true distinction between a parameter "value" named "key", and parameter "key=value"
  ...
  begin
    ...
    if fs.addFileGUI(f, parent, True) = NIL then
      f.free
    else
      spaceIf(TRUE)
  end; // addFolder
  ...
  begin
    ...
    if not pars.parExist('var') then // will we work with a variable?
      ...
      begin
        ...
        if i < 0 then
          exit; // this var doesn't exit. don't write.
        encode := FALSE;
...
//      s := pars.parEx('password');
      if pars.parExistVal('password', s) then
...
//      s := pars.parEx('newname');
      if pars.parExistVal('newname', s) then
  ...
  begin
    ...
    if s = 'redirect' then
      result:=a.redir
     else if s = 'no limits' then
      trueIf(a.nolimits)
     else if s = 'enabled' then
      trueIf(a.enabled)
     else if s = 'is group' then
      trueIf(a.group)
     else if s = 'member of' then
      result:=join(';',a.link)
     else if s = 'notes' then
      result:=a.notes
     else if s = 'password' then
      result:=a.pwd
     else if s = 'password is' then
      trueIf((a.pwd=pars[2]) or (trim(a.pwd)=par(2)))  //add by mars
     else if s = 'exists' then
      result:='1';
  ...
  function fromTable(tbl: String; const k: String): String;
  var
    i: integer;
    space, h: THashedStringList;
    s: string;
  begin
    result:='';
    if tbl = 'ini' then
      deprecatedMacro('from table|ini','from table|#ini');
    try
    ...
    if stringExists(tbl, ['ini','#ini']) and (result = 'no') then
      result:='';
  end; // fromTable
  ...
  procedure actionAllowed(const action:TfileAction);
  var
    f: Tfile;
    s: String;
    local: boolean;
  ...
  procedure cookie();
    ...
    function timeForCookies(const v: String): String;
    var
      t: Tdatetime;
    begin
      try
...
//    i: integer;
  ...
  // search for the symbol in the translation table
    ...
    if ansiStartsText('%sym-', name) then // legacy: surpassed by {.section.}
      ...
      if assigned(md.f) and assigned(md.tpl) and md.f.hasRecursive(FA_ARCHIVABLE) then
        result := md.tpl['item-archive']
       else
        result := ''
      ...
      if md.f = NIL then
        ...
        result := intToStr(md.f.DLcount)
    else if name = '%connections%' then
      result:=intToStr(fs.htSrv.conns.count)
      ...
      result:=floatToStrF(fs.htSrv.speedOut/1000, ffFixed, 7,2)
    else if name = '%speed-in%' then
      result:=floatToStrF(fs.htSrv.speedIn/1000, ffFixed, 7,2)
    else if name = '%total-out%' then
      result:=smartSize(outTotalOfs + fs.htSrv.bytesSent)
    else if name = '%total-in%' then
      result:=smartSize(inTotalOfs + fs.htSrv.bytesReceived)
      ...
      result := fs.htSrv.port
     else
      lFound := false;
    if not lFound and assigned(md.cd) then
      begin
      usr := md.cd.usr;
      lFound := True;
      if name = '%host%' then
    ...
    if not lFound and assigned(md.folder) then
      begin
        lFound := True;
    ...
    if not lFound and assigned(md.f) then
...
begin
  try
    ...
    try
      ...
      if (spDisableMacros in sp) then
        exit(fullMacro);
      if (spNonLocalIPDisableMacros in sp) and Assigned(md) and Assigned(md.cd) and (md.cd.address > '') and not md.cd.isLocalAddress then
        exit(fullMacro);
...
//          end;
      ...
      if Assigned(fs.macroFuncs) and fs.macroFuncs.ContainsKey(name)  then
        begin
          var ff: TMacroFunc;
          if fs.macroFuncs.TryGetValue(name, ff) then
            Result := ff.func(fs, fullMacro, pars, cbData);
          Exit;
        end;
      ...
      if name = 'add to log' then
        ...
        fs.add2log(s, md.cd, stringToColorEx(par(1,'color'), clDefault));
        result:='';
        end
       else
      if name = 'mkdir' then
  ...
  //        result:=noMacrosAllowed(UTF8toAnsi(p))
      ...
      if name = 'length' then
        begin // don't trim
          var v: String;
          if pars.parExistVal('var', v, False) then
           try
             s := getVar(v)
            except
             s := pars[0]
      ...
      if name = 'get' then
        try
          ...
          else if p = 'can upload' then
             actionAllowed(FA_UPLOAD)
          else if p = 'can delete' then
             actionAllowed(FA_DELETE)
          else if p = 'can access' then
             actionAllowed(FA_ACCESS)
          else if p = 'can archive' then
             canArchive(md.folder)
          else if p = 'can archive item' then
             canArchive(md.f)
          else if p = 'has thumbnail' then
             hasThumbnail(md.f)
          else if p = 'url' then
             getUri()
          else if p = 'stop spiders' then
             trueIf(spStopSpiders in SP)
          else if p = 'is new' then
             trueIf(md.f.isNew())
        ...
        begin
          if satisfied(md.cd) then
            try
              ...
              fs.htSrv.limiters.add(md.cd.limiter);
              result:='';
             except
              md.cd.conn.limiters.remove(md.cd.limiter);
              fs.htSrv.limiters.remove(md.cd.limiter);
...
//       else
//      if name = 'delete item' then // moved to HFS.Macroses!!!
//        deleteItem()
        ;

      if pars.count < 2 then exit; // from here, only macros with at least 2 parameters

//      if name = 'set item' then // moved to HFS.Macroses!!!
//        setItem()
//       else
